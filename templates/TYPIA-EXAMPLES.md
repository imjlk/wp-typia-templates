...
1. [기본 타입 정의](#기본-타입-정의)
2. [자동 블록 메타데이터 생성](#자동-블록-메타데이터-생성)  
3. [런타임 검증](#런타임-검증)
...
### sync-types-to-block-json.ts 래퍼
...
이 래퍼는 공용 TypeScript Compiler API 분석기를 호출해서 두 가지 산출물을 생성합니다.

- `block.json`: WordPress가 직접 소비하는 최소 메타데이터
- `typia.manifest.json`: `format`, `pattern`, 길이/범위, 중첩 구조까지 보존한 full constraint manifest
...
**출력 (`block.json` + `typia.manifest.json`):**
