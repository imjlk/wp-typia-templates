...
const questions = [
  {
    name: 'title',
    message: 'Block title (e.g., My Interactive Block):',
    default: 'My Typia Interactive Block',
    transform: (val) => val.trim()
  },
  {
    name: 'description',
    message: 'Block description:',
    default: 'An interactive WordPress block with Typia validation and WordPress Interactivity API',
    transform: (val) => val.trim()
  },
  {
    name: 'author',
  ...
  {
    name: 'slug',
    message: 'Block slug (e.g., my-interactive-block):',
...
async function setup() {
  console.log('🚀 Setting up your WordPress Typia Interactive Block\n');
  ...
  try {
    ...
    for (const [key, value] of Object.entries(packageJson.scripts)) {
      newScripts[key] = typeof value === 'string'
        ? value.replace(/wp-typia-interactivity/g, templateVars.slugSnakeCase)
        : value;
    }
    ...
    console.log('\n🎮 Your interactive block is ready!');
    console.log('   • Click the block to interact');
    console.log('   • Try different animation modes');
    console.log('   • Check the counter and progress bar');
    console.log('\n🎉 Happy coding with Typia and Interactivity API!');
...
if (process.env.npm_config_global) {
  ...
  console.log('   Use: npx wp-typia-interactivity');
  process.exit(1);
}
