...
test("runtime bridge delegates output and sync helpers to focused modules", () => {
	...
	expect(bridgeSource).not.toContain("export async function executeSyncCommand(");
	expect(outputSource).toContain("export function printCompletionPayload(");
	expect(outputSource).toContain("export function buildCreateCompletionPayload(");
	expect(outputSource).toContain("export function buildMigrationCompletionPayload(");
