...
const questions = [
  {
    name: 'title',
    message: 'Block title (e.g., My Advanced Block):',
    default: 'My Typia Advanced Block',
    transform: (val) => val.trim()
  },
  {
    name: 'description',
    message: 'Block description:',
    default: 'An advanced WordPress block with Typia validation, migrations, and future PHP validation foundations',
    transform: (val) => val.trim()
  },
  {
    name: 'author',
...
async function setup() {
  console.log('🚀 Setting up your WordPress Typia Advanced Block\n');
  ...
  try {
    ...
    for (const [key, value] of Object.entries(packageJson.scripts)) {
      newScripts[key] = typeof value === 'string'
        ? value.replace(/wp-typia-advanced/g, templateVars.slugSnakeCase)
        : value;
    }
    ...
    console.log('\n🧭 Migration tooling and metadata generation are ready to go.');
    console.log('🎉 Happy coding with Typia validation!');
...
if (process.env.npm_config_global) {
  ...
  console.log('   Use: npx wp-typia-advanced');
  process.exit(1);
}
