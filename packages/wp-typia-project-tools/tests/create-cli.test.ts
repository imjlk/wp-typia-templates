...
async function runGeneratedScriptAsync(
	targetDir: string,
...
function replaceGeneratedTransportBaseUrls(transportPath: string, baseUrl: string) {
...
function runGeneratedJsonScript(
	targetDir: string,
	scriptName: string,
	scriptSource: string,
) {
...
async function runGeneratedJsonScriptAsync(
	targetDir: string,
	scriptName: string,
	scriptSource: string,
) {
...
describe("@wp-typia/project-tools scaffolding", () => {
	...
	test(
		...
		expect(readme).toContain("`src/transport.ts` is the first-class transport seam");
		...
		expect(readme).toContain("Edit `src/transport.ts` when you need to switch between direct WordPress REST and a contract-compatible proxy or BFF");
...
console.log(JSON.stringify({
	...
	test(
		...
		async () => {
			const targetDir = path.join(tempRoot, "demo-persistence-adapter-transport");
...
console.log(JSON.stringify({ initial, updated, reread }));
					...
					initial: { count: number; postId: number; resourceKey: string; storage: string };
					reread: { count: number; postId: number; resourceKey: string; storage: string };
					updated: { count: number; postId: number; resourceKey: string; storage: string };
				};
	...
	test(
		...
		const generatedTransport = fs.readFileSync(
			path.join(targetDir, "src", "blocks", "demo-compound-storage", "transport.ts"),
			"utf8",
		);
		const parentEdit = fs.readFileSync(
		...
		expect(readme).toContain('npm run add-child -- --slug faq-item --title "FAQ Item"');
		expect(readme).toContain("## PHP REST Extension Points");
		expect(readme).toContain("Edit `src/blocks/demo-compound-storage/transport.ts` when you need to switch between direct WordPress REST and a contract-compatible proxy or BFF");
	...
	test("compound scaffolds enable public persistence when only a public policy is provided", async () => {
		...
		const generatedTransport = fs.readFileSync(
			path.join(targetDir, "src", "blocks", "demo-compound-public", "transport.ts"),
			"utf8",
		);
		const restPublicHelper = fs.readFileSync(path.join(targetDir, "inc", "rest-public.php"), "utf8");
