...
function isManifestTypiaMetadata(
  value: unknown,
): value is ManifestTypiaMetadata {
  return (
    isPlainObject(value) &&
...
export function isManifestAttribute(
  value: unknown,
): value is ManifestAttribute {
  return (
    isPlainObject(value) &&
...
export function assertManifestDocument<TDocument = ManifestDocument>(
  value: unknown,
): TDocument {
...
export function parseManifestDocument<TDocument = ManifestDocument>(
  value: unknown,
): TDocument {
 ...
 *
 * @param path - Dot-separated manifest attribute path.
 * @param attribute - Manifest attribute metadata for that path.
 * @param options - Label and control preferences for editor generation.
 * @returns A normalized field descriptor for editor rendering helpers.
 * @example
 * ```ts
...
export function describeEditorField(
  ...
  return {
    ...
    minimum: isNumber(attribute.typia.constraints.minimum)
      ...
      : null,
    // Preserve the legacy descriptor contract used by older generated helpers:
    // enum options are only exposed for select controls.
...
/**
 * Build the editor model for every visible manifest attribute.
 *
 * @param manifest - Parsed manifest document used to derive field metadata.
 * @param options - Label and visibility preferences for editor generation.
 * @returns A flat list of editor field descriptors.
 * @example
 * ```ts
