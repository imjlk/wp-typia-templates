...
async function readCounter(
  ...
  return previewPage.evaluate(async (routeContext) => {
    const restRoot =
      (window as typeof window & { wpApiSettings?: { root?: string } }).wpApiSettings?.root ||
      document.querySelector<HTMLLinkElement>('link[rel="https://api.w.org/"]')?.href ||
      `${window.location.origin}/wp-json/`;

    const buildUrl = (root: string, routePath: string, params?: Record<string, string>) => {
    ...
    const response = await fetch(
      buildUrl(restRoot, '/persistence-examples/v1/counter', {
        postId: String(routeContext.postId),
...
async function writeCounter(
  ...
  return previewPage.evaluate(async (payload) => {
    const restRoot =
      (window as typeof window & { wpApiSettings?: { root?: string } }).wpApiSettings?.root ||
      document.querySelector<HTMLLinkElement>('link[rel="https://api.w.org/"]')?.href ||
      `${window.location.origin}/wp-json/`;

    const buildUrl = (root: string, routePath: string) => {
      const normalizedRoute = routePath.endsWith('/') ? routePath : `${routePath}/`;
      const url = new URL(root);
...
async function readLikeStatus(
  ...
  return previewPage.evaluate(async (routeContext) => {
    ...
    if (typeof routeContext.restNonce === 'string' && routeContext.restNonce.length > 0) {
      headers['X-WP-Nonce'] = routeContext.restNonce;
    }

    const response = await fetch(
      buildUrl(restRoot, '/persistence-examples/v1/likes', {
        postId: String(routeContext.postId),
        resourceKey: routeContext.resourceKey,
      }),
      {
...
async function toggleLikeDirect(
  ...
  return previewPage.evaluate(async ({ routeContext, withNonceHeader }) => {
    const restRoot =
      (window as typeof window & { wpApiSettings?: { root?: string } }).wpApiSettings?.root ||
      document.querySelector<HTMLLinkElement>('link[rel="https://api.w.org/"]')?.href ||
      `${window.location.origin}/wp-json/`;
    ...
    if (withNonceHeader && typeof routeContext.restNonce === 'string') {
      headers['X-WP-Nonce'] = routeContext.restNonce;
    }
  ...
  }, { routeContext: context, withNonceHeader: withNonce });
}
