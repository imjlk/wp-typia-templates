...
function manifestMatchesAttribute(attribute: ManifestAttribute, value: unknown): boolean {
	...
	switch (attribute.ts.kind) {
		...
		case "array":
			return Array.isArray(value) && Boolean(attribute.ts.items) && value.every((item) => manifestMatchesAttribute(attribute.ts.items as ManifestAttribute, item));
		case "object":
			return isPlainObject(value) && Object.entries(attribute.ts.properties ?? {}).every(([key, property]) =>
...
function matchesUnionAttribute(attribute: ManifestAttribute, value: unknown): boolean {
	...
	return typeof branchKey === "string" && branchKey in branches && manifestMatchesAttribute(branches[branchKey], value);
}
