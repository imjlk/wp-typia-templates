    ...
    <ErrorBoundary>
      <InspectorControls>
        ...
        >
          <RangeControl
            label={
              paddingTopField?.label ?? __("Padding Top", "my_typia_block")
            }
            value={padding.top}
            min={paddingTopField?.minimum ?? 0}
            max={64}
            step={paddingTopField?.step ?? 1}
            onChange={(value) => updatePadding("top", value ?? padding.top)}
          />
          <RangeControl
            label={
              paddingRightField?.label ?? __("Padding Right", "my_typia_block")
            }
            value={padding.right}
            min={paddingRightField?.minimum ?? 0}
            max={64}
            step={paddingRightField?.step ?? 1}
            onChange={(value) => updatePadding("right", value ?? padding.right)}
          />
          <RangeControl
            label={
          ...
          <RangeControl
            label={
              paddingLeftField?.label ?? __("Padding Left", "my_typia_block")
            }
            value={padding.left}
            min={paddingLeftField?.minimum ?? 0}
            max={64}
            step={paddingLeftField?.step ?? 1}
            onChange={(value) => updatePadding("left", value ?? padding.left)}
          />
          {!isValid && (
