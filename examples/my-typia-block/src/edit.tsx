...
export default function Edit({ attributes, setAttributes }: EditProps) {
	...
	const updateAttribute = <K extends keyof MyTypiaBlockAttributes>(
		...
		if (validation.success) {
			setAttributes({ [key]: value } as Partial<MyTypiaBlockAttributes>);
		} else {
	...
	return (
		<ErrorBoundary>
			<InspectorControls>
				<PanelBody title={__('Settings', 'my_typia_block')}>
					...
					<SelectControl
						...
						options={[
							{ label: __('Left', 'my_typia_block'), value: 'left' },
							{ label: __('Center', 'my_typia_block'), value: 'center' },
							{ label: __('Right', 'my_typia_block'), value: 'right' },
							{ label: __('Justify', 'my_typia_block'), value: 'justify' },
						]}
					...
					<SelectControl
						...
						options={[
							{ label: __('Small', 'my_typia_block'), value: 'small' },
							{ label: __('Medium', 'my_typia_block'), value: 'medium' },
							{ label: __('Large', 'my_typia_block'), value: 'large' },
							{ label: __('Extra Large', 'my_typia_block'), value: 'xlarge' },
						]}
						onChange={(fontSize) => updateAttribute('fontSize', fontSize as any)}
					/>
					{!isValid && (
						<div className="components-notice is-error">
							<p><strong>{__('Validation Errors:', 'my_typia_block')}</strong></p>
				...
				<PanelBody title={__('Migration Manager', 'my_typia_block')} initialOpen={false}>
					<MigrationDashboard />
				</PanelBody>
			</InspectorControls>
			...
			<div {...blockProps} className={classNames(blockProps.className, {
				...
				<div className="my-typia-block-editor">My Typia Block - Editor View</div>
				<RichText
					tagName="p"
					value={attributes.content}
					onChange={(content) => updateAttribute('content', content)}
					placeholder={__('Enter your text...', 'my_typia_block')}
					className={classNames('my-typia-block-content', `align-${attributes.alignment}`)}
					style={{
				...
				<div className="block-info">
					<small>
						{__('My Typia Block – showcase of Typia validation, migrations, and interactivity.', 'my_typia_block')}
					</small>
				</div>
			</div>
		</ErrorBoundary>
