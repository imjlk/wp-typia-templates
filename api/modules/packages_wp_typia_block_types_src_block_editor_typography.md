[WordPress Typia Boilerplate - v1.0.0](../README.md) / [Modules](../modules.md) / packages/wp-typia-block-types/src/block-editor/typography

# Module: packages/wp-typia-block-types/src/block-editor/typography

## Table of contents

### Type Aliases

- [TextTransform](packages_wp_typia_block_types_src_block_editor_typography.md#texttransform)
- [TextDecoration](packages_wp_typia_block_types_src_block_editor_typography.md#textdecoration)
- [FontStyle](packages_wp_typia_block_types_src_block_editor_typography.md#fontstyle)
- [WritingMode](packages_wp_typia_block_types_src_block_editor_typography.md#writingmode)

### Variables

- [TEXT\_TRANSFORMS](packages_wp_typia_block_types_src_block_editor_typography.md#text_transforms)
- [TEXT\_DECORATIONS](packages_wp_typia_block_types_src_block_editor_typography.md#text_decorations)
- [FONT\_STYLES](packages_wp_typia_block_types_src_block_editor_typography.md#font_styles)
- [WRITING\_MODES](packages_wp_typia_block_types_src_block_editor_typography.md#writing_modes)

## Type Aliases

### TextTransform

Ƭ **TextTransform**: ``"none"`` \| ``"capitalize"`` \| ``"uppercase"`` \| ``"lowercase"``

Derived from Gutenberg text-transform control values.

#### Defined in

[packages/wp-typia-block-types/src/block-editor/typography.ts:4](https://github.com/imjlk/wp-typia/blob/main/packages/wp-typia-block-types/src/block-editor/typography.ts#L4)

___

### TextDecoration

Ƭ **TextDecoration**: ``"none"`` \| ``"underline"`` \| ``"line-through"``

Derived from Gutenberg text-decoration control values.

#### Defined in

[packages/wp-typia-block-types/src/block-editor/typography.ts:16](https://github.com/imjlk/wp-typia/blob/main/packages/wp-typia-block-types/src/block-editor/typography.ts#L16)

___

### FontStyle

Ƭ **FontStyle**: ``"normal"`` \| ``"italic"``

Derived from Gutenberg font appearance controls.

#### Defined in

[packages/wp-typia-block-types/src/block-editor/typography.ts:27](https://github.com/imjlk/wp-typia/blob/main/packages/wp-typia-block-types/src/block-editor/typography.ts#L27)

___

### WritingMode

Ƭ **WritingMode**: ``"horizontal-tb"`` \| ``"vertical-rl"`` \| ``"vertical-lr"``

Derived from Gutenberg writing mode control values.

#### Defined in

[packages/wp-typia-block-types/src/block-editor/typography.ts:37](https://github.com/imjlk/wp-typia/blob/main/packages/wp-typia-block-types/src/block-editor/typography.ts#L37)

## Variables

### TEXT\_TRANSFORMS

• `Const` **TEXT\_TRANSFORMS**: readonly [``"none"``, ``"capitalize"``, ``"uppercase"``, ``"lowercase"``]

#### Defined in

[packages/wp-typia-block-types/src/block-editor/typography.ts:6](https://github.com/imjlk/wp-typia/blob/main/packages/wp-typia-block-types/src/block-editor/typography.ts#L6)

___

### TEXT\_DECORATIONS

• `Const` **TEXT\_DECORATIONS**: readonly [``"none"``, ``"underline"``, ``"line-through"``]

#### Defined in

[packages/wp-typia-block-types/src/block-editor/typography.ts:18](https://github.com/imjlk/wp-typia/blob/main/packages/wp-typia-block-types/src/block-editor/typography.ts#L18)

___

### FONT\_STYLES

• `Const` **FONT\_STYLES**: readonly [``"normal"``, ``"italic"``]

#### Defined in

[packages/wp-typia-block-types/src/block-editor/typography.ts:29](https://github.com/imjlk/wp-typia/blob/main/packages/wp-typia-block-types/src/block-editor/typography.ts#L29)

___

### WRITING\_MODES

• `Const` **WRITING\_MODES**: readonly [``"horizontal-tb"``, ``"vertical-rl"``, ``"vertical-lr"``]

#### Defined in

[packages/wp-typia-block-types/src/block-editor/typography.ts:39](https://github.com/imjlk/wp-typia/blob/main/packages/wp-typia-block-types/src/block-editor/typography.ts#L39)
