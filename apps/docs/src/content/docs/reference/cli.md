...
| `--route-pattern <route-pattern>`                                | Generated REST resource item route pattern, relative to the namespace. Named regex captures must include `(?P<id>...)` so generated handlers and clients stay aligned.         |
